...
export default function Login() {
  const [email, setEmail] = useState("");
    ...
    const handleSubmit = async (e) => {
      ...
      axios.post("http://localhost:3001/login", { email, password /* image */} )
    ...
    <div className="flex justify-center items-center h-screen ">
      <div className="w-[500px] shadow-lg p-10">
        <form className="flex flex-col text-md gap-1" onSubmit={handleSubmit}>
          ...
          <CustomInputField name="Email" type="email" placeholder="Enter your Email" value= {email} onChange={(e) => setEmail(e.target.value)} />
          <CustomInputField name="Password" type="password" placeholder="Enter your Password" value= {password} onChange={(e) => setPassword(e.target.value)} />
          <button type="submit" className="bg-black text-white p-2 rounded mt-5">Login</button>
          <p className="text-center pt-2">Dont have an Account? <span className="text-blue-600 cursor-pointer" onClick={() => navigate("/signup")}>Sign Up</span></p>
        </form>
      </div>
    </div>
  )
